...
+(NSString *)getDeviceHardware{
   ...
   free(machine);
   NSString* hw = [self platformType:platform];
    if ([hw isEqualToString:@"Simulator"])
    {
        machine = (char*)getenv("SIMULATOR_MODEL_IDENTIFIER");
...
+(BOOL)isIphoneX {
    ...
    if ([[self platformType:[self getDeviceHardware]] isEqualToString:@"iPhone X"]){
